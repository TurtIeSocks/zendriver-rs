...
| -------------- | -------- | ------------------------------------------------------------- | ------------------------------------- |
| `stealth`      | yes      | Anti-detection: spoofed UA/platform, full-surface WebGL parameter spoof resolved from measured GPU capability tiers, a 482-device catalogue to name or draw a specific GPU, isolated worlds, JS shim, opt-in `gpu_backend` for real-GPU WebGL/WebGPU rendering | (built-in to `zendriver`)             |
...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `zendriver-mcp` | Drive a stealth Chrome from any LLM agent — [Model Context Protocol](https://modelcontextprotocol.io/) server, 72 tools, stdio + streamable HTTP | `cargo install zendriver-mcp` ([docs](https://turtiesocks.github.io/zendriver-rs/mcp.html)) |
...
4. **`Tab`/`Browser` completeness** — multi-tab, cookies, storage, frames, nav history, `wait_for_idle`, per-context isolation with first-class per-context proxy + auth (`browser.browser_context().proxy(...).proxy_auth(...).build()`). See [multi-tab](https://turtiesocks.github.io/zendriver-rs/multi-tab.html) + [frames](https://turtiesocks.github.io/zendriver-rs/frames.html) + [browser-context](https://turtiesocks.github.io/zendriver-rs/browser-context.html).
...
| MCP server for AI agents | yes (`zendriver-mcp`, 72 tools) | no        | no              | no              | no              |
