...
| `GpuBackendUnavailable` | Chrome never advertised its WS endpoint on a launch that requested [`GpuBackend::Native`](https://docs.rs/zendriver/latest/zendriver/enum.GpuBackend.html) — typically no GPU present, a crashed GPU process, or a missing GPU sandbox. | See [GPU backend](./gpu-backend.md#no-automatic-fallback). Catch this error and retry with `GpuBackend::SwiftShader` or `GpuBackend::Disabled`; zendriver does not fall back automatically. |
