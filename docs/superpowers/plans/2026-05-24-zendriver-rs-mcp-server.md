...
| `zendriver::Cookie + SameSite` derive `JsonSchema` | **don't** — only `Serialize + Deserialize` (lib doesn't depend on `schemars`). For MCP wire schemas: mirror with `CookieDto` + `SameSiteDto` in `tools/cookies.rs` (same field set + serde shape, plus `JsonSchema`); add `From<Cookie>` / `From<CookieDto>` + same for `SameSite`. On-disk JSON (cookies_persist) stays identical to what the lib emits because the serde shape matches exactly. |
...
| `Element::press(impl Into<String>)` | takes typed `zendriver::Key` enum. **`Key` is a two-variant enum: `Char(char)` and `Special(SpecialKey)`** — NOT a flat enum of named keys. Single-char strings → `Key::Char(c)`; special-key names route through `SpecialKey` (`Enter / Tab / Escape / Backspace / Delete / Space / ArrowUp/Down/Left/Right / Home / End / PageUp / PageDown / Insert / CapsLock / NumLock / ScrollLock / PrintScreen / Pause / ContextMenu / F1..F12`). **No `Key::from_str` impl** — write a `parse_key(&str) -> Result<Key, ErrorData>` matching lowercase special names + falling through to `Key::Char(c)` on single-char input. |
| `Element::click_with(ClickOptions)` | **`ClickOptions { button: MouseButton, modifiers: KeyModifiers, click_count: u32, force: bool, realistic: bool, position: Option<(f64, f64)> }`** — full field set verified in `crates/zendriver/src/element/actions.rs:71`. `Default` is left-button single-click, realistic Bezier path, no force / no modifiers / no position offset. MCP layer maps `Option<button>` + `Option<click_count>` onto the relevant `ClickOptions` fields and leaves the rest at default — `modifiers / force / realistic / position` are v0+1 territory. |
| `MouseButton` variant set | actually **5 variants**: `Left / Middle / Right / Back / Forward` (defined at `crates/zendriver/src/input/mouse.rs:23`). MCP layer's `MouseButtonArg` only exposes the 3 commonly-dispatched ones (Left/Middle/Right); thumb buttons are deliberately omitted from the wire schema to keep it small. |
| `KeyModifiers` variant set | **`bitflags!` struct with `ALT / CTRL / META / SHIFT`** (at `crates/zendriver/src/input/keyboard.rs:151`) — combine via `|`. `KeyModifiers::empty()` for the no-mod case. Not currently surfaced over the MCP wire (no per-action `modifiers` field in v0); reserved for follow-up. |
