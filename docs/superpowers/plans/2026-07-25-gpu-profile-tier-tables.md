...
- Produces:
  ...
  - generated `pub(crate) static BASE_PARAMS_WEBGL2`, `PARAM_OVERRIDES`, `PRECISION`, `EXTENSIONS_WEBGL1`, `EXTENSIONS_WEBGL2` in `crates/zendriver-stealth/src/gpu/tiers.rs`
...
```rust
    ...
    fn shared_values_go_to_base_and_only_differences_become_overrides() {
        ...
        let (base, overrides) = split_base_and_overrides(&[a, b]);
...
pub fn split_base_and_overrides(
    ...
    let all_names: std::collections::BTreeSet<&String> =
        tiers.iter().flat_map(|t| t.params_webgl2.keys()).collect();

    for name in all_names {
        let present: Vec<&ParamValue> =
            tiers.iter().filter_map(|t| t.params_webgl2.get(name)).collect();
        ...
        if universal && identical {
            ...
            for t in tiers {
                if let Some(v) = t.params_webgl2.get(name) {
                    overrides
                        .get_mut(&t.name)
...
pub fn emit_rust(tiers: &[TierData]) -> String {
    let (base, overrides) = split_base_and_overrides(tiers);
    ...
    s.push_str("/// Values every tier agrees on. Sorted, binary-searchable.\n");
    s.push_str("pub(crate) static BASE_PARAMS_WEBGL2: &[(&str, GlParam)] = &[\n");
    for (k, v) in &base {
        s.push_str(&format!("    ({k:?}, {}),\n", lit(v)));
    }
    s.push_str("];\n\n");

    s.push_str("/// Per-tier exceptions to the base, keyed by tier name.\n");
    s.push_str("pub(crate) static PARAM_OVERRIDES: &[(&str, &[(&str, GlParam)])] = &[\n");
...
/// Flatten the base table plus one tier's overrides into a profile.
pub(crate) fn profile_for_tier(tier: Tier) -> GpuProfile {
    let key = tier_key(tier);
    let mut params: BTreeMap<String, GlParam> = tiers::BASE_PARAMS_WEBGL2
        .iter()
        .map(|(k, v)| ((*k).to_string(), v.to_owned_param()))
        .collect();
    if let Some(over) = lookup(tiers::PARAM_OVERRIDES, key) {
        for (k, v) in *over {
            params.insert((*k).to_string(), v.to_owned_param());
        }
    }
    ...
    // WebGL1's params are the WebGL2 set minus the entries WebGL1 has no enum
    // for; the generated per-context extension lists carry the real split.
    GpuProfile {
        provenance: Provenance::Probed {
            chrome: "see data/gpu-tiers".into(),
            os: "see data/gpu-tiers".into(),
        },
        params_webgl1: params.clone(),
        params_webgl2: params,
        precision,
