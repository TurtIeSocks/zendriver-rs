...
impl<'tab> ImpervaBypass<'tab> {
    ...
    /// No-args: the bypass driver spins up its own `InterceptBuilder::subscribe`
    /// against the tab session. Attaching to an existing `InterceptHandle` is
    /// not viable because `subscribe` consumes a fresh `InterceptBuilder`.
    pub fn with_interception(self) -> Self;
