...
impl ZendriverServer {
    ...
    #[tool(
        ...
        description = "Drive the Cloudflare Turnstile clearance flow on the current tab. Locates the Turnstile iframe, clicks the checkbox at the default 15%/50% bbox offset, then polls every `poll_interval_ms` (default 500ms) until one of three terminal states is reached, bounded by `timeout_ms` (default 30_000 = 30s): `solved` (a token input picked up a value — returned in `token`), `challenge_gone` (the widget stopped being actionable without a token, e.g. clearance cookie shortcut), or `timeout` (deadline elapsed — not an error, the agent can retry or fall back). Cloudflare owns this markup and moves it without notice, so both halves are request fields rather than baked in: `selectors` sets the markers to hunt for (iframe src substring, container, token inputs) and `click_policy` sets whether, how often and where to click (`max_attempts: 0` watches without touching). Override them when the defaults have stopped matching a page instead of waiting on a release. Errors only on structural failures: CDP call failed, or in-page JS exception."
    )]
