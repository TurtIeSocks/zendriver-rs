...
/// Pull the single JSON member out of the corpus zip.
///
/// # Errors
/// If the archive cannot be read or holds no `.json` member.
pub fn unzip_network_json(zipped: &[u8]) -> Result<String, Box<dyn std::error::Error>> {
    let mut archive = zip::ZipArchive::new(std::io::Cursor::new(zipped))?;
    let name = (0..archive.len())
        .filter_map(|i| archive.by_index(i).ok().map(|f| f.name().to_string()))
        .find(|n| n.ends_with(".json"))
        .ok_or("corpus archive holds no .json member")?;
    let mut out = String::new();
    std::io::Read::read_to_string(&mut archive.by_name(&name)?, &mut out)?;
    Ok(out)
}

/// Render the committed catalogue module.
///
/// Data only: `CatalogueEntry` itself is hand-written in `gpu::types`, the same
/// split `gpu-tier-gen` uses for `tiers.rs`. A generated file that also defines
/// its own types cannot be reviewed for shape independently of its contents.
///
/// Renderer strings are **not** stored. They are composed at read time from
/// ANGLE's format, so a Chrome format change is one fix in one function rather
/// than a rewrite of every row.
#[must_use]
pub fn emit_rust(corpus_commit: &str, pci_ids_commit: &str, report: &BuildReport) -> String {
    let mut out = String::new();
    out.push_str("// GENERATED by `cargo run -p gpu-catalogue-gen`. DO NOT EDIT.\n");
    out.push_str("//\n");
    out.push_str("// Identity only. Every capability value still comes from the measured\n");
    out.push_str("// tier tables in `tiers.rs`; nothing here can fabricate one.\n");
    out.push_str("//\n");
    out.push_str(&format!(
        "// Model names, device ids and population weights: fingerprint corpus\n\
         // pinned at {corpus_commit}.\n\
    ...
    if !report.unmatched.is_empty() {
        out.push_str(&format!(
            "// {} models were dropped for having no device id in either source,\n\
